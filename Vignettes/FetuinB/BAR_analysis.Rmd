...
### Too noisy to plot every protein. Summarize trend with mean.
...
Hypergeometric enrichment of the 2 clusters
Cluster 1
```{r}
...
View(data.table(gseaGO_CC@result)[p.adjust <= 0.1 ,])
```
...
gseaGO_BP <- gseGO(geneList = LFC_geneName,
                   ...
                   pAdjustMethod = 'BH',
                   verbose = F) %>% clusterProfiler::simplify(cutoff = 0.8, by="p.adjust", select_fun=min)
View(data.table(gseaGO_BP@result)[p.adjust <= 0.1 ,])
...
gseaGO_MF <- gseGO(geneList = LFC_geneName,
                   ...
                   pAdjustMethod = 'BH',
                   verbose = F) %>% clusterProfiler::simplify(cutoff = 0.8, by="p.adjust", select_fun=min)
View(data.table(gseaGO_MF@result)[p.adjust <= 0.1 ,])
...
gseaReactome <- gsePathway(geneList = LFC_entrez,
                           ...
                           pAdjustMethod = 'BH',
                           verbose = F) %>% setReadable(OrgDb=org.Hs.eg.db, "ENTREZID")
View(data.table(gseaReactome@result)[p.adjust <= 0.1 ,])
```


save GSEA results in excel workbook to cherry pick from
```{r}
allGSEA <- list("KEGG"=data.table(gseaKEGG@result)[p.adjust <= 0.1 ,], "Reactome"=data.table(gseaReactome@result)[p.adjust <= 0.1 ,], "GO_CC"=data.table(gseaGO_CC@result)[p.adjust <= 0.1 ,], "GO_BP"=data.table(gseaGO_BP@result)[p.adjust <= 0.1 ,], "GO_MF"=data.table(gseaGO_MF@result)[p.adjust <= 0.1 ,])
