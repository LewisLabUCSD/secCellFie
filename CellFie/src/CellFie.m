function[score, score_binary ,taskInfos, detailScoring, geneScore_table]=CellFie(data,SampleNumber,ref,param, pathway, sampNames)
...
%   pathway                     choice of pathways used in analysis: 'metabolism',
%                               'secretion', or 'metabolism_secretion'
%
% OUTPUTS:
...
if pathway=="secretion"
    load('taskStructure_sec');
elseif pathway=="metabolism"
    load('taskStructure')
    taskInfos=struct2cell(taskStructure);
    taskInfos=taskInfos';
    taskInfos(:,5:end)=[];
else
    error('Pathway not valid')
end
...
if pathway=="secretion"
    load(strcat('essentialRxns/essentialRxnsbyTask_',ref, '_sec'));
elseif pathway=="metabolism"
...
geneScore=cat(2, str2double(data.gene), Gene_score);
geneScore_table=array2table(geneScore, 'VariableNames', sampNames);
...
if pathway=="metabolism"
    ...
    load(strcat('parsedGPR/parsedGPR_',ref, '_sec'));
end
